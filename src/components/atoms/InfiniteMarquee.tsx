        ...
        <div className="inline-flex items-center">
          ...
          {items.map((item, i) => (
            <div key={`first-${i}`} className="inline-flex items-center px-12">
              <span className="text-xl md:text-2xl font-serif italic text-text-primary/60 hover:text-heritage transition-colors duration-500 cursor-default uppercase tracking-widest font-light">
                {item}
              </span>
              <span className="mx-12 w-2 h-2 rounded-full bg-burnished/20" />
            </div>
          ))}
